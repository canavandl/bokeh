// Annular Wedge shader for WebGPU rendering
// Implements a pie slice with a hole (combination of annulus and wedge)
...
struct VertexOutput {
    ...
    @location(3) v_start_angle: f32,
    @location(4) v_end_angle: f32,
    @location(5) v_linewidth: f32,
    @location(6) v_line_color: vec4f,
    @location(7) v_fill_color: vec4f,
    @location(8) v_line_cap: f32,
    @location(9) v_line_join: f32,
}
...
fn vertex_main(
    ...
    // Calculate vertex position - uses outer radius for bounding box
...
// ============================================================================
// SDF Functions for Annular Wedge
// ============================================================================
...
// Annular wedge SDF = intersection of annulus and wedge
fn annular_wedge_sdf(p: vec2f, inner_radius: f32, outer_radius: f32, start_angle: f32, end_angle: f32) -> f32 {
    return max(
        annulus_sdf(p, outer_radius, inner_radius),
        wedge_sdf(p, outer_radius, start_angle, end_angle)
    );
}
...
fn fragment_main(input: VertexOutput) -> @location(0) vec4f {
    let dist = annular_wedge_sdf(
        input.v_coords,
        input.v_inner_radius,
        input.v_outer_radius,
        input.v_start_angle,
        input.v_end_angle
    );

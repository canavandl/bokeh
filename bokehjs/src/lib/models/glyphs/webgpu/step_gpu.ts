// WebGPU implementation for Step glyph
...
export class StepGPU extends BaseGPUGlyph {
  ...
  constructor(wrapper: WebGPUWrapper, override readonly glyph: StepView) {
    super(wrapper, glyph)
  }

  draw(indices: number[], main_glyph: GlyphView, transform: Transform): void {
    const main_step = main_glyph as StepView
    const main_gpu = main_step.gpuglyph
    ...
    this._draw_step(indices, transform, main_gpu)
  }

  private _draw_step(_indices: number[], transform: Transform, main_gpu: StepGPU): void {
    const {webgpu_wrapper} = this
    const device = webgpu_wrapper.device

    // Calculate number of step points/segments
    const npoints = main_gpu.glyph.sx.length
    const mode = main_gpu.glyph.model.mode
    const nstep_points = mode === "center" ? 3 * npoints - 2 : 2 * npoints - 1
    const nsegments = nstep_points > 1 ? nstep_points - 1 : 0
    ...
    this._ensure_pipeline_and_buffers(device, webgpu_wrapper)
  ...
  private _ensure_pipeline_and_buffers(device: GPUDevice, wrapper: WebGPUWrapper): void {
  ...
  private _update_data(nsegments: number): void {
    const nstep_points = nsegments + 1
    ...
    const step_points: Array<{x: number, y: number}> = []

    let is_finite = isFinite(sx[0] + sy[0])
    step_points.push({x: is_finite ? sx[0] : NaN, y: sy[0]})

    for (let i = 0; i < npoints - 1; i++) {
      const next_finite = isFinite(sx[i + 1] + sy[i + 1])
      switch (mode) {
        case "before":
          step_points.push({x: is_finite && next_finite ? sx[i] : NaN, y: sy[i + 1]})
          step_points.push({x: next_finite ? sx[i + 1] : NaN, y: sy[i + 1]})
          break
        case "after":
          step_points.push({x: is_finite && next_finite ? sx[i + 1] : NaN, y: sy[i]})
          step_points.push({x: next_finite ? sx[i + 1] : NaN, y: sy[i + 1]})
          break
        case "center":
          if (is_finite && next_finite) {
            const midx = (sx[i] + sx[i + 1]) / 2
            step_points.push({x: midx, y: sy[i]})
            step_points.push({x: midx, y: sy[i + 1]})
            step_points.push({x: sx[i + 1], y: sy[i + 1]})
          } else {
            step_points.push({x: is_finite ? sx[i] : NaN, y: sy[i]})
            step_points.push({x: NaN, y: NaN})
            step_points.push({x: next_finite ? sx[i + 1] : NaN, y: sy[i + 1]})
          }
          break
        default:
    ...
    const is_closed = nstep_points > 2 &&
                      step_points[0].x === step_points[nstep_points - 1].x &&
                      step_points[0].y === step_points[nstep_points - 1].y &&
                      isFinite(step_points[0].x + step_points[0].y)

    // Pre-compute finite flags
    const finite: boolean[] = new Array(nstep_points)
    for (let i = 0; i < nstep_points; i++) {
      finite[i] = isFinite(step_points[i].x + step_points[i].y)
    }
    ...
    for (let seg = 0; seg < nsegments; seg++) {
      ...
      this._point_start_data[seg * 2] = step_points[i].x
      this._point_start_data[seg * 2 + 1] = step_points[i].y
      this._point_end_data[seg * 2] = step_points[j].x
      this._point_end_data[seg * 2 + 1] = step_points[j].y
      ...
      if (i > 0) {
        ...
        prev_idx = nstep_points - 2
      } else {
        prev_idx = i
      }
      this._point_prev_data[seg * 2] = step_points[prev_idx].x
      this._point_prev_data[seg * 2 + 1] = step_points[prev_idx].y
      ...
      this._point_next_data[seg * 2] = step_points[next_idx].x
      this._point_next_data[seg * 2 + 1] = step_points[next_idx].y
      ...
      this._show_flags_data[seg * 4 + 3] = 0
    }
  }
  ...
  private _update_visuals(nsegments: number): void {
    ...
    // Get scalar values (Step uses LineScalar)

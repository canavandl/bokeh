// WebGPU implementation for LRTB glyph (used by quad, hbar, vbar, block, etc.)
...
export class LRTBGPU extends BaseGPUGlyph {
  ...
  constructor(wrapper: WebGPUWrapper, override readonly glyph: LRTBView) {
    super(wrapper, glyph)
  }
  ...
  draw(indices: number[], main_glyph: GlyphView, transform: Transform): void {
    const main_lrtb = main_glyph as LRTBView
    const main_gpu = main_lrtb.gpuglyph
    ...
    this._draw_lrtb(indices, transform, main_gpu)
  }

  private _draw_lrtb(_indices: number[], transform: Transform, main_gpu: LRTBGPU): void {
  ...
  private _update_uniforms(transform: Transform, border_radius: [number, number, number, number]): void {
    // For LRTB, we use screen coordinates directly, so scale factors are 1
    ...
    // Identity transform (screen coordinates)
    this._uniform_data[8] = 1.0   // x_scale
    this._uniform_data[9] = 0.0   // x_offset
    this._uniform_data[10] = 1.0  // y_scale
    this._uniform_data[11] = 0.0  // y_offset
  ...
  private _update_data(nmarkers: number): void {
    ...
    const lrtb = this.glyph as LRTBLikeView

    // Get screen coordinates
    const {sleft, sright, stop, sbottom, border_radius} = lrtb
    ...
    for (let i = 0; i < nmarkers; i++) {
      ...
      if (!isFinite(l + r + t + b)) {
        ...
        size_data[i * 2] = MISSING_POINT
        size_data[i * 2 + 1] = MISSING_POINT
      } else {
        // Compute center from LRTB
        pos_data[i * 2] = (l + r) / 2
        pos_data[i * 2 + 1] = (t + b) / 2

        // Size from LRTB
        size_data[i * 2] = abs(r - l)
        size_data[i * 2 + 1] = abs(t - b)
      }

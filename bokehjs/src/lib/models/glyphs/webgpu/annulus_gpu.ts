// WebGPU implementation for Annulus glyph
...
export class AnnulusGPU extends BaseGPUGlyph {
  ...
  constructor(wrapper: WebGPUWrapper, override readonly glyph: AnnulusView) {
    super(wrapper, glyph)
  }

  draw(indices: number[], main_glyph: GlyphView, transform: Transform): void {
    const main_annulus = main_glyph as AnnulusView
    const main_gpu = main_annulus.gpuglyph
    ...
    this._draw_annuli(indices, transform, main_gpu)
  }

  private _draw_annuli(_indices: number[], transform: Transform, main_gpu: AnnulusGPU): void {
  ...
  private _ensure_pipeline(device: GPUDevice, wrapper: WebGPUWrapper): void {
    ...
    if (this._pipeline == null) {
      this._pipeline = wrapper.get_annulus_pipeline()
    }

    if (this._bind_group == null) {
      this._bind_group = wrapper.create_annulus_bind_group(this._uniform_buffer)
    }
  }
  ...
  private _upload_buffer(
    ...
    data: Float32Array,
    target: AnnulusGPU,
    buffer_name: "_position_buffer" | "_radii_buffer" | "_line_props_buffer" | "_line_color_buffer" | "_fill_color_buffer",
  ): void {
  ...
  private _update_data(nmarkers: number): void {
    ...
    const {x, y, inner_radius, outer_radius} = this.glyph

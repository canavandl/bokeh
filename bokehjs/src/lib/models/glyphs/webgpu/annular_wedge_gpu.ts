// WebGPU implementation for AnnularWedge glyph
...
export class AnnularWedgeGPU extends BaseGPUGlyph {
  private readonly _antialias: number = 1.5

  // GPU buffers
  private _position_buffer: GPUBuffer | null = null
  private _radii_buffer: GPUBuffer | null = null
  private _angles_buffer: GPUBuffer | null = null
  ...
  private _radii_data: Float32Array | null = null
  private _angles_data: Float32Array | null = null
  ...
  private _radii_upload_needed: boolean = true
  private _angles_upload_needed: boolean = true
  ...
  constructor(wrapper: WebGPUWrapper, override readonly glyph: AnnularWedgeView) {
    super(wrapper, glyph)
  }

  draw(indices: number[], main_glyph: GlyphView, transform: Transform): void {
    const main_annular = main_glyph as AnnularWedgeView
    const main_gpu = main_annular.gpuglyph
    ...
    this._draw_annular_wedges(indices, transform, main_gpu)
  }

  private _draw_annular_wedges(_indices: number[], transform: Transform, main_gpu: AnnularWedgeGPU): void {
    ...
    if (main_gpu._data_dirty || size_changed) {
      ...
      main_gpu._radii_upload_needed = true
      main_gpu._angles_upload_needed = true
    }
    ...
    if (main_gpu._radii_upload_needed) {
      this._upload_buffer(device, main_gpu._radii_data!, main_gpu, "_radii_buffer")
      main_gpu._radii_upload_needed = false
    }
    if (main_gpu._angles_upload_needed) {
      this._upload_buffer(device, main_gpu._angles_data!, main_gpu, "_angles_buffer")
      main_gpu._angles_upload_needed = false
    }
    ...
    render_pass.setVertexBuffer(2, main_gpu._radii_buffer)
    render_pass.setVertexBuffer(3, main_gpu._angles_buffer)
  ...
  private _ensure_pipeline(device: GPUDevice, wrapper: WebGPUWrapper): void {
    ...
    if (this._pipeline == null) {
      this._pipeline = wrapper.get_annular_wedge_pipeline()
    }

    if (this._bind_group == null) {
      this._bind_group = wrapper.create_annular_wedge_bind_group(this._uniform_buffer)
    }
  }
  ...
  private _upload_buffer(
    device: GPUDevice,
    data: Float32Array,
    target: AnnularWedgeGPU,
    buffer_name: "_position_buffer" | "_radii_buffer" | "_angles_buffer" | "_line_props_buffer" | "_line_color_buffer" | "_fill_color_buffer",
  ): void {
  ...
  private _update_data(nmarkers: number): void {
    ...
    this._radii_data = this._ensure_array(this._radii_data, nmarkers * 2)
    this._angles_data = this._ensure_array(this._angles_data, nmarkers * 2)

    const pos_data = this._position_data
    const radii_data = this._radii_data
    const angles_data = this._angles_data

    const {x, y, inner_radius, outer_radius, start_angle, end_angle} = this.glyph
    const direction = this.glyph.model.direction
    ...
    for (let i = 0; i < nmarkers; i++) {
      ...
      radii_data[i * 2] = inner_radius.get(i)
      radii_data[i * 2 + 1] = outer_radius.get(i)

      // Handle direction - swap angles for clockwise
      if (direction === "anticlock") {
        angles_data[i * 2] = start_angle.get(i)
        angles_data[i * 2 + 1] = end_angle.get(i)
      } else {
        angles_data[i * 2] = end_angle.get(i)
        angles_data[i * 2 + 1] = start_angle.get(i)
      }
    }
  }
  ...
  destroy(): void {
    ...
    this._radii_buffer?.destroy()
    this._angles_buffer?.destroy()

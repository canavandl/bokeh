// WebGPU implementation for Wedge glyph
...
export class WedgeGPU extends BaseGPUGlyph {
  ...
  constructor(wrapper: WebGPUWrapper, override readonly glyph: WedgeView) {
    super(wrapper, glyph)
  }

  draw(indices: number[], main_glyph: GlyphView, transform: Transform): void {
    const main_wedge = main_glyph as WedgeView
    const main_gpu = main_wedge.gpuglyph
    ...
    this._draw_wedges(indices, transform, main_gpu)
  }

  private _draw_wedges(_indices: number[], transform: Transform, main_gpu: WedgeGPU): void {
  ...
  private _ensure_pipeline(device: GPUDevice, wrapper: WebGPUWrapper): void {
    ...
    if (this._pipeline == null) {
      this._pipeline = wrapper.get_wedge_pipeline()
    }

    if (this._bind_group == null) {
      this._bind_group = wrapper.create_wedge_bind_group(this._uniform_buffer)
    }
  }
  ...
  private _upload_buffer(
    ...
    data: Float32Array,
    target: WedgeGPU,
    buffer_name: "_position_buffer" | "_radius_buffer" | "_angles_buffer" | "_line_props_buffer" | "_line_color_buffer" | "_fill_color_buffer",
  ): void {
  ...
  private _update_data(nmarkers: number): void {
    ...
    const {x, y, radius, start_angle, end_angle} = this.glyph

...
export class CircleGPU extends BaseGPUGlyph {
  ...
  private _draw_markers(_indices: number[], transform: Transform, main_gpu: CircleGPU): void {
    ...
    // Update data buffers only when actual data changes (NOT on pan/zoom!)
    ...
    // Update visual buffers (changes on visual change)
    ...
    // For WebGPU with GPU-side transforms, we're fast enough to always render all markers.
    // Skip the LOD system that would hide markers during interaction.
    // Only update show flags on size change (to initialize all markers as visible).
    if (size_changed) {
      // Show all markers - we don't need LOD with GPU-side transforms
      main_gpu._update_show_flags_all(nmarkers)
      main_gpu._line_props_upload_needed = true
    }

    // Upload only dirty buffers to GPU
  ...
  private _update_visuals(nmarkers: number): void {
    ...
    for (let i = 0; i < nmarkers; i++) {
      ...
      // Line props: linewidth, cap, join, show (show set later)
  ...
  // Set all markers as visible (used for WebGPU which doesn't need LOD)

...
/**
 * Manages test execution including baseline validation, screenshot comparison, and retry logic.
 */
export class TestRunner {
  constructor(
    private browser: BrowserManager,
    private ctx: TestRunContext,
  ...
  /**
   * Runs a test case with retry logic if enabled.
   *
   * @param test_case - The test case to run (suites, test, status)
   * @param retry - Whether to enable retry logic for failed tests
   */
  ...
  private async run_test(suites: Suite[], test: Test, status: TestCase[2], attempt: number | null): Promise<boolean> {
    ...
    const ctx_ = JSON.stringify(this.ctx)

    let output: Value<Result> | Failure | Timeout
    try {
      output = await this.execute_test(seq, ctx_, test)
    } catch (error) {
  ...
  private async execute_test(seq: string, ctx_: string, test: Test): Promise<Value<Result> | Failure | Timeout> {
    ...
    do {
      const output = await (async () => {
        ...
        try {
          return await this.browser.evaluate<Result>(`Tests.run(${seq}, ${ctx_})`)
        } finally {
          if (test.dpr != null || test.scale != null) {
